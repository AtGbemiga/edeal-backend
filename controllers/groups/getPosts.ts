  ...
  if (!user_id) {
    try {
      pool.execute<RowDataPacket[]>(
        ...
        u.account_name AS owner_firstname, 
        u.img AS owner_img, 
        COUNT(epl.fk_user_id) AS likes,
        (SELECT COUNT(*) FROM egroup_post_likes WHERE fk_group_post_id = egp.id AND fk_user_id = 0) AS user_liked,
        (SELECT JSON_ARRAYAGG(
          JSON_OBJECT(
            ...
            "commentator_first_name", uc.account_name
          )
